...
Buffers polygons delimite areas that are within some distance of input features. We can have them arround points or lines. We will see here examples of both. 
"
...
md"
Next we will draw geodesic circles with 500 km radius with center on the orthodrome vertices. And to do it we need ofc to compute those circles. We do that with the `cirgeo` function. 
"
...
md"
Now imagine that we plot many close circles and compute the union of them all. That's when we get a geodesic buffer.
"
...
# and plot it
coast(R=:d, proj=(name=:ortho, center=(0,45)), land=:peru, frame=:g, plot=(data=D, fill=:green), show=true)
