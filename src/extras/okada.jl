...
function helper_okd_grid(G::GMTgrid, x_start::Float64, y_start::Float64, W::Float64, L::Float64, depth::Float64, strike::Float64, dip::Float64)
	...
	return x, y, W, L, depth, x_centroid, y_centroid
end
...
#-------------------------------------------------------------------------------------------------------
function okada_opt_parser(fault_start::Bool; kw...)
	# Do all the parsings here so that only this tinny function gets recompiled when any of kwargs change.
	msg = ""
	x_start = NaN; y_start = NaN; depth = NaN; strike = NaN; dip = NaN; L = NaN; W = NaN; rake = NaN; slip = NaN
	if (fault_start)
		(x = get(kw, :x_start, nothing)) === nothing ? (msg = "'x_start' must be specified") : (x_start = Float64(x))
		(y = get(kw, :y_start, nothing)) === nothing ? (msg = "'y_start' must be specified") : (y_start = Float64(y))
	end
	(d = get(kw, :depth, nothing))   === nothing ? (msg = "'depth' must be specified")   : (depth = Float64(d))
	(s = get(kw, :strike, nothing))  === nothing ? (msg = "'strike' must be specified")  : (strike = Float64(s))
	(d = get(kw, :dip, nothing))     === nothing ? (msg = "'dip' must be specified")     : (dip = Float64(d))
	(l = get(kw, :L, nothing))       === nothing ? (msg = "'L' must be specified")       : (L = Float64(l))
	(w = get(kw, :W, nothing))       === nothing ? (msg = "'W' must be specified")       : (W = Float64(w))
	(r = get(kw, :rake, nothing))    === nothing ? (msg = "'rake' must be specified")    : (rake = Float64(r))
	(s = get(kw, :slip, nothing))    === nothing ? (msg = "'slip' must be specified")    : (slip = Float64(s))
	nu = Float64(get(kw, :nu, 0.25))
	U3 = Float64(get(kw, :open, 0.0))
	do_all3 = (get(kw, :enz, nothing) !== nothing)
	do_z3d  = (get(kw, :z3d, nothing) !== nothing)
	return msg, x_start, y_start, depth, strike, dip, L, W, rake, slip, nu, U3, do_all3, do_z3d
end
...
function okada(x::Vector{Float64}, y::Vector{Float64}; kw...)
	...
	msg, _, _, depth, strike, dip, L, W, rake, slip, nu, U3, do_all3, = okada_opt_parser(false; kw...)
	(msg != "") && error(msg)

	if (do_all3)  okada_enz(x, y, W, L, depth, strike, dip, rake, slip, nu, U3)
	else          okada_z(  x, y, W, L, depth, strike, dip, rake, slip, nu, U3)
	end
end

#-------------------------------------------------------------------------------------------------------
function okada_geog_z(e::Vector{Float64}, n::Vector{Float64}, W, L, depth, strike, dip, rake, slip, nu, xc, yc, U3)::Matrix{Float32}
	...
	if (U3 == 0.0)
		@inbounds Threads.@threads for col = 1:length(e)
			@inbounds for row = 1:length(n)
				...
				uz[row,col] = okd_uz(U1, U2, x, p, q, L, W, sin_dip, cos_dip, nu)
			end
		end
	else
		...
		@inbounds Threads.@threads for col = 1:length(e)
			@inbounds for row = 1:length(n)
				e1, n1, _ = geodetic2enu(e[col], n[row], 0.0, xc, yc, 0.0)
				x, p, q = xy2okada(e1, n1, sin_dip, cos_dip, sin_strike, cos_strike, L, ct1, ct2, ct3, ct4, ct5)
				uz[row,col] = okd_uz(U1, U2, U3, x, p, q, L, W, sin_dip, cos_dip, nu)
			end
		end
	end
	return uz
...
# -------------------------------------------------------------------------------------------------------
function okada_geog_z3d(G::GMTgrid{Float32,2}, e::Vector{Float64}, n::Vector{Float64}, W, L, depth, strike, dip, rake, slip, nu, xc, yc)
	# This function computes also the contribution of the horizontal deformations into the vertical component.
	# I saw this in the https://github.com/CyprienBosserelle/Tsunami.jl

	U1, U2, sin_dip, cos_dip, sin_strike, cos_strike, ct1, ct2, ct3, ct4, ct5 = preokada(depth, strike, dip, rake, slip, W)
	#zzz = Matrix{Float32}(undef, length(n), length(e))
	#_ue, _un, _uz = Vector{Float32}(undef, length(n)), Vector{Float32}(undef, length(n)), Vector{Float32}(undef, length(n))
	ue, un, uz = Matrix{Float32}(undef, length(n), length(e)), Matrix{Float32}(undef, length(n), length(e)), Matrix{Float32}(undef, length(n), length(e))
	dHdx, dHdy = fill(0.f0, length(n), length(e)), fill(0.f0, length(n), length(e))
	#_dHdx, _dHdy = fill(0.f0, length(n)), fill(0.f0, length(n))

	ef = Matrix{Float32}(undef, length(n), length(e))
	nf = Matrix{Float32}(undef, length(n), length(e))
	@inbounds Threads.@threads for col = 1:length(e)
		@inbounds for row = 1:length(n)
			ef[row,col], nf[row,col], _ = geodetic2enu(e[col], n[row], 0.0, xc, yc, 0.0)
		end
	end

	dHdx[:,2:end] = diff(G.z, dims=2) ./ diff(ef, dims=2)
	dHdy[2:end,:] = diff(G.z, dims=1) ./ diff(nf, dims=1)
	@inbounds Threads.@threads for col = 1:length(e)
		@inbounds for row = 1:length(n)
			#e1, n1, _ = geodetic2enu(e[col], n[row], 0.0, xc, yc, 0.0)
			#x, p, q = xy2okada(e1, n1, sin_dip, cos_dip, sin_strike, cos_strike, L, ct1, ct2, ct3, ct4, ct5)
			x, p, q = xy2okada(ef[row,col], nf[row,col], sin_dip, cos_dip, sin_strike, cos_strike, L, ct1, ct2, ct3, ct4, ct5)
			ue[row,col], un[row,col], uz[row,col] = helper_okada2_enz(U1, U2, x, p, q, L, W, sin_dip, cos_dip, sin_strike, cos_strike, nu)
			#_ue[row], _un[row], _uz[row] = helper_okada2_enz(U1, U2, x, p, q, L, W, sin_dip, cos_dip, sin_strike, cos_strike, nu)
		end
		#_dHdy[2:end] .= diff(view(G.z, :, col)) ./ diff(view(nf, :, col))
		#zzz[:,col]  .= _uz .+ _un .* _dHdy .+ _ue .* view(dHdx, :, col)
	end
	uz .+ ue .* dHdx .+ un .* dHdy
	#return zzz
end
...
#-------------------------------------------------------------------------------------------------------
# The non-geog case.
function okada_z(e::Vector{Float64}, n::Vector{Float64}, W, L, depth, strike, dip, rake, slip, nu, U3)::Matrix{Float32}
	U1, U2, sin_dip, cos_dip, sin_strike, cos_strike, ct1, ct2, ct3, ct4, ct5 =
		preokada(depth, strike, dip, rake, slip, W)
	uz = Matrix{Float32}(undef, length(n), length(e))
	if (U3 == 0.0)
...
function preokada(depth, strike, dip, rake, slip, W)
	...
	sin_dip, cos_dip       = sind(dip), cosd(dip)
	sin_strike, cos_strike = sind(strike), cosd(strike)

	# Defines dislocation in the fault plane system
	U1 = cosd(rake) * slip / (2*pi)		# Move the "/(2*pi)" term from ukd_u?? to here
	U2 = sind(rake) * slip / (2*pi)
