...
function parse_theme_names(name::String)
	...
	if (name[1] == 'A')			# Name should either start with A2 or A0
		...
		if contains(name, "Graph")							# Add a vector to the end of each axis
			gmtlib_setparameter(API, "MAP_FRAME_TYPE", "graph")
		...
		if (name[2] == '0')
			...
			else                    gmtlib_setparameter(API, "MAP_FRAME_PEN", "0.001,white@100") # No axes, but need this sad trick
			end
		end
...
function fonts_colors_settings(fonts, colors, bg_color)
	...
	gmtlib_setparameter(API, "FONT_LABEL", "auto,$(fonts[1]),$(colors[1])")
