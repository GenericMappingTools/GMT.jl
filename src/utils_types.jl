...
function imagesc(mat::Union{GMTgrid,Matrix{<:AbstractFloat}}; x=Float64[], y=Float64[], hdr=Float64[],
	             proj4::String="", wkt::String="", GI::Union{GItype,Nothing}=nothing, clim=[0,255], cmap=GMTcpt(), kw...)::GMTimage
...
function imagesc(I::GMTimage{<:UInt16}; clim=0)::GMTimage
...
function mat2img(mat, G::GMTgrid; names::Vector{String}=String[], metadata::Vector{String}=String[])
	...
	GMTimage(G.proj4, G.wkt, G.epsg, G.geog, range, copy(G.inc), G.registration, NaN32, "Gray", metadata, names, copy(G.x), copy(G.y), zeros(size(mat,3)), mat, zeros(Int32,3), String[], 0, Array{UInt8,2}(undef,1,1), length(G.layout) == 4 ? G.layout : G.layout*"a", 0)
end
...
function slicecube(GI::GItype; slice::Int=0, α=0.0, angle=0.0, axis="x", cmap=GMTcpt())
	...
	# Returns a GMTgrid, a GMTimage (in case GI is an GMTimage), or two GMTgrids otherwise.
	_axis = lowercase(string(axis))
