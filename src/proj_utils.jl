...
@static Sys.iswindows() ?
	(Sys.WORD_SIZE == 64 ? (const libproj = "proj_w64") : (const libproj = "proj_w32")) : (
		Sys.isapple() ? (const libproj = Symbol(split(readlines(pipeline(`otool -L $(GMT.libgdal)`, `grep libproj`))[1])[1])) : (
			Sys.isunix() ? (const libproj = Symbol(split(readlines(pipeline(`ldd $(GMT.libgdal)`, `grep libproj`))[1])[3])) :
