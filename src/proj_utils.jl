...
function geodesic_long(lonlat1::VMr, lonlat2::VMr; step=0.0, np=180, proj::String="", epsg::Integer=0)
	...
	_name = TMPDIR_USR.dir * "/" * "GMTtmp_" * TMPDIR_USR.username * TMPDIR_USR.pid_suffix * ".dat" 	# Due to bug in mapproject need to save in tmp file
...
function helper_geoglimits(prj::String, region::Vector{Float64})::Vector{Float64}
	...
	o1 = Gdal.gdalvectortranslate(ds, opts; dest="/vsimem/tmp", gdataset=true)::Gdal.IDataset
	ds = Gdal.get_gdaldataset([region[2] region[4]], opts, false)[1]
	o2 = Gdal.gdalvectortranslate(ds, opts; dest="/vsimem/tmp", gdataset=true)::Gdal.IDataset
