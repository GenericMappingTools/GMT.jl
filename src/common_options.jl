...
function genFun(this_key::Symbol, user_input::NamedTuple, mapa::NamedTuple)::String
	...
	for k = 1:length(user_input)
		if (haskey(d, key[k]))
			val = d[key[k]]
			if (isa(val, Function))
				(val == add_opt_fill) && (out[1] *= val(Dict(key[k] => user_input[key[k]])) )
			else
...
function common_grd(d::Dict, cmd0::String, cmd::String, prog::String, args...)
	...
	(arg1 !== nothing && isa(arg1, Array{<:Number}) && startswith(prog, "grd")) && (arg1 = mat2grid(arg1))
...
function dbg_print_cmd(d::Dict, cmd)
	...
	(show_kwargs[1]) && (show_kwargs[1] = false; return "")	# If in HELP mode
