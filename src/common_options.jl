...
function parse_common_opts(d::Dict, cmd::String, opts::VMs; first::Bool=true, is3D::Bool=false)::Tuple{String, String}
	...
	for opt in opts
		...
		elseif (opt == :q)  cmd, o = parse_q(d, cmd)
...
function helper_decorated(d::Dict, compose=false)
	...
	elseif ((val = find_in_dict(d, [:locations])[1]) !== nothing)
		(isa(val, AbstractString)) && (cmd = val)
		optD = "f"
	end
	if (cmd == "")
...
function guessgeog(GI::GItype)::Bool
	...
	(GI.range[1] >= -180 && GI.range[2] <= 360) && GI.range[3] >= -90 && GI.range[4] <= 90
end
function guessgeog(D::GDtype)::Bool
	(isa(D, GMTdataset) && D.proj4 != "") && return isgeog(D)
	(isa(D, Vector) && D[1].proj4 != "") && return isgeog(D)
...
function dbg_print_cmd(d::Dict, cmd::Vector{String})
	...
	if ( ((Vd = find_in_dict(d, [:Vd])[1]) !== nothing) || CONVERT_SYNTAX[1])
		...
		if (Vd >= 2)					# Delete these first before reporting
			...
			#GMT.CURRENT_CPT[1] = GMTcpt()		# Can't do this because it would f. plot(..., colorbar=true)
		end
		if (length(d) > 0)
