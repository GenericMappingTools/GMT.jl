"""
	convert(cmd0::String="", arg1=[], kwargs...)

Convert, Paste, and/or Extract columns from data tables

Full option list at [`convert`](http://gmt.soest.hawaii.edu/doc/latest/convert.html)
...
- **F** : **conn_method** : -- Str or [] --   Flags = [c|n|r|v][refpoint]
    ...
    [`-F`](http://gmt.soest.hawaii.edu/doc/latest/convert.html#f)
...
- **S** : **select_hdr** : -- Str --      Flags =  [~]”search string” or [~]/regexp/[i]
    ...
    [`-S`](http://gmt.soest.hawaii.edu/doc/latest/convert.html#s)
- **T** : **suppress ** : -- Str or [] --    Flags = [h|d]
    ...
    [`-T`](http://gmt.soest.hawaii.edu/doc/latest/convert.html#t)
- **W** : **convert** : -- Str or [] --      Flags =  [+n]

    Attempt to convert each word in the trialing text to a number and append such values
    to the numerical output columns.
    [`-W`](http://gmt.soest.hawaii.edu/doc/latest/convert.html#w)
- **Z** : **range** : -- Str or [] --      Flags =  [first][:last]
    ...
    [`-Z`](http://gmt.soest.hawaii.edu/doc/latest/convert.html#z)
- $(GMT.opt_V)
- $(GMT.opt_b)
- $(GMT.opt_d)
- $(GMT.opt_e)
...
- $(GMT.opt_swap_xy)
"""
function convert(cmd0::String="", arg1=[]; kwargs...)

	length(kwargs) == 0 && occursin(" -", cmd0) && return monolitic("convert", cmd0, arg1)	# Speedy mode
	...
	cmd = add_opt(cmd, 'W', d, [:W :convert])
	cmd = add_opt(cmd, 'Z', d, [:Z :range])

	cmd, got_fname, arg1 = find_data(d, cmd0, cmd, 1, arg1)
	return common_grd(d, cmd, got_fname, 1, "convert", arg1)		# Finish build cmd and run it
end

# ---------------------------------------------------------------------------------------------------
convert(arg1=[], cmd0::String=""; kw...) = convert(cmd0, arg1; kw...)