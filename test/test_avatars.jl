	...
	plot(mat2ds([1:200 rand(200)]), decimate=true, Vd=dbg2)
	...
	x = LinRange(0,2π,50);
	lines(x, sin.(x), ls="FrontSlip", legend=true, figname="lixo3", theme=("A2"))
	lines(x, sin.(x), ls="lineCirc", legend=(label="sin(x)", box=:none), lc=:red, figname="lixo3")
	lines(x, sin.(x), ls="FrontCircLeft", legend=(label="sin(x)"), mc=:red, figname="lixo3")
