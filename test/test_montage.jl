@testset "MONTAGE" begin
	...
	a = ones(2, 3)
	w, h = subplot_panel_sizes(a, total_width=12.0)
	@test length(w) == 3
	@test length(h) == 2
	...
	a = [2.0 0.5;
	     2.0 0.5]
	w, h = subplot_panel_sizes(a, total_width=20.0)
	...
	# Single panel
	a = reshape([1.5], 1, 1)
	w, h = subplot_panel_sizes(a, total_width=15.0)
	...
	w, h = subplot_panel_sizes([I1, I2], ncols=2, total_width=20.0)
	@test length(w) == 2
	@test length(h) == 1
