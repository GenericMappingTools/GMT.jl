@testset "COMMON OPTS" begin
	# -------------------- Test common_options ----------------------------------------
	GMT.dict2nt(Dict{Symbol,Any}(:a =>1, :b => 2))
	@test GMT.parse_R(Dict{Symbol,Any}(:xlim => (1,2), :ylim => (3,4), :zlim => (5,6)), "")[1] == " -R1/2/3/4/5/6"
	@test GMT.parse_R(Dict{Symbol,Any}(:R => (1.0,2.0,3.0,4.0), :ylim => (33,44), :zlim => (5,6)), "")[1] == " -R1.0/2.0/33/44/5/6"
	@test GMT.parse_R(Dict{Symbol,Any}(:region_llur => (1,2,3,4)), "")[1] == " -R1/3/2/4+r"
	...
	@test GMT.arg2str(Dict{Symbol,Any}(:shaded => "-4p/-6p/grey20@40"), [:shaded]) == "-4p/-6p/grey20@40"
	@test GMT.arg2str(Dict{Symbol,Any}(:shaded => "aa bb"), [:shaded]) == "\"aa bb\""
	@test_throws ErrorException("arg2str: argument 'arg' can only be a String, Symbol, Number, Array or a Tuple.") GMT.arg2str(typeof(1))
	@test GMT.parse_b(Dict{Symbol,Any}(:b => (ncols=2, swapp_bytes=true, little_endian=true, type=:char)), "")[2] == " -b2cw+l"
	@test GMT.parse_b(Dict{Symbol,Any}(:b => (ncols=2, type=:double)), "")[2] == " -b2d"
	@test GMT.parse_b(Dict{Symbol,Any}(:b => (ncols=2, type=:ai)), "")[2] == " -b2d"
	@test GMT.parse_bi(Dict{Symbol,Any}(:bi => (ncols=2, type=:float)), "")[2] == " -bi2f"
	@test GMT.parse_bo(Dict{Symbol,Any}(:bo => (ncols=2, type=:int16)), "")[2] == " -bo2h"
	@test GMT.parse_c(Dict{Symbol,Any}(:c => (1,2)), "")[1] == " -c0,1"
	@test GMT.parse_c(Dict{Symbol,Any}(:c => [1,2]), "")[1] == " -c0,1"
	@test GMT.parse_c(Dict{Symbol,Any}(:c => "1,2"), "")[1] == " -c0,1"
	@test GMT.parse_c(Dict{Symbol,Any}(:c => 1), "")[1] == " -c0"
	@test GMT.parse_c(Dict{Symbol,Any}(:c => "1"), "")[1] == " -c0"
	@test GMT.parse_l(Dict{Symbol,Any}(:l => "ai ai"), "")[2] == " -l\"ai ai\""
	@test GMT.parse_l(Dict{Symbol,Any}(:l => (text="ai ai", vspace=3)), "")[2] == " -l\"ai ai\"+G3"
	@test GMT.parse_n(Dict{Symbol,Any}(:n => (bicubic=true,antialiasing=true,bc=:g)), "")[2] == " -nc+bg"
	@test GMT.parse_I(Dict{Symbol,Any}(:inc => (x=1.5, y=2.6, unit="meter")),"", [:I :inc], "I", true) == " -I1.5e/2.6e"
	@test GMT.parse_I(Dict{Symbol,Any}(:inc => (x=1.5, y=2.6, unit="m")),"", [:I :inc], "I", true) == " -I1.5m/2.6m"
	@test GMT.parse_I(Dict{Symbol,Any}(:inc => (x=1.5, y=2.6, unit="data")),"", [:I :inc], "I", true) == " -I1.5/2.6u"
	@test GMT.parse_I(Dict{Symbol,Any}(:inc => (x=1.5, y=2.6, extend="data")),"", [:I :inc], "I", true) == " -I1.5+e/2.6+e"
	@test GMT.parse_I(Dict{Symbol,Any}(:inc => (x=1.5, y=2.6, unit="nodes")),"", [:I :inc], "I", true) == " -I1.5+n/2.6+n"
	@test GMT.parse_I(Dict{Symbol,Any}(:inc => (2,4)),"", [:I :inc], "I", true) == " -I2/4"
	@test GMT.parse_I(Dict{Symbol,Any}(:inc => [2 4]),"", [:I :inc], "I", true) == " -I2/4"
	@test GMT.parse_I(Dict{Symbol,Any}(:inc => "2"),"", [:I :inc], "I", true) == " -I2"
	@test GMT.parse_I(Dict{Symbol,Any}(:inc => "2"),"", [:I :inc], "", true) == "2"
	@test GMT.parse_JZ(Dict{Symbol,Any}(:JZ => "5c"), "")[1] == " -JZ5c"
	@test GMT.parse_JZ(Dict{Symbol,Any}(:Jz => "5c"), "")[1] == " -Jz5c"
	@test GMT.parse_JZ(Dict{Symbol,Any}(:aspect3 => 1), " -JX10")[1] == " -JX10 -JZ10"
	@test GMT.parse_J(Dict{Symbol,Any}(:J => "X5"), "", default="", map=false)[1] == " -JX5"
	@test GMT.parse_J(Dict{Symbol,Any}(:a => ""), "", default="", map=true, O=true)[1] == " -J"
	@test GMT.parse_J(Dict{Symbol,Any}(:J => "X", :figsize => 10), "")[1] == " -JX10"
	@test GMT.parse_J(Dict{Symbol,Any}(:J => "X", :scale => "1:10"), "")[1] == " -Jx1:10"
	@test GMT.parse_J(Dict{Symbol,Any}(:proj => "Ks0/15"), "")[1] == " -JKs0/15"
	@test GMT.parse_J(Dict{Symbol,Any}(:scale=>"1:10"), "")[1] == " -Jx1:10"
	@test GMT.parse_J(Dict{Symbol,Any}(:s=>"1:10"), "", default=" -JU")[1] == " -JU"
	@test GMT.parse_J(Dict{Symbol,Any}(:J => "Merc", :figsize => 10), "", default="", map=true, O=true)[1] == " -JM10"
	@test GMT.parse_J(Dict{Symbol,Any}(:J => "+proj=merc"), "")[1] == " -J+proj=merc+width=" * split(GMT.DEF_FIG_SIZE, '/')[1]
	@test GMT.parse_J(Dict{Symbol,Any}(:J => (name=:albers, parallels=[45 65])), "", default="", map=false)[1] == " -JB0/0/45/65"
	@test GMT.parse_J(Dict{Symbol,Any}(:J => (name=:albers, center=[10 20], parallels=[45 65])), "", default="", map=false)[1] == " -JB10/20/45/65"
	@test GMT.parse_J(Dict{Symbol,Any}(:J => "winkel"), "", default="", map=false)[1] == " -JR"
	@test GMT.parse_J(Dict{Symbol,Any}(:J => "M0/0"), "", default="", map=false)[1] == " -JM0/0"
	@test GMT.parse_J(Dict{Symbol,Any}(:J => (name=:merc,center=10)), "",default="", map=false)[1] == " -JM10"
	@test GMT.parse_J(Dict{Symbol,Any}(:J => (name=:merc,parallels=10)), "",default="", map=false)[1] == " -JM0/0/10"
	...
	@test GMT.parse_w(Dict{Symbol,Any}(:w => (10,2)), "")[1] == " -wc10/2"
	@test GMT.parse_w(Dict{Symbol,Any}(:w => "+c2"), "")[1] == " -w+c2"
	@test GMT.parse_w(Dict{Symbol,Any}(:w => (period=10,)), "")[1] == " -wc10"
	@test GMT.parse_w(Dict{Symbol,Any}(:w => (period=(10,2))), "")[1] == " -wc10/2"
	@test GMT.parse_w(Dict{Symbol,Any}(:w => (period=(10,2), col=3)), "")[1] == " -wc10/2+c3"
	@test GMT.opt_pen(Dict{Symbol,Any}(:lw => 5, :lc => :red),'W', [:a]) == " -W5,red"
	@test GMT.opt_pen(Dict{Symbol,Any}(:lw => 5),'W', [:a]) == " -W5"
	@test GMT.opt_pen(Dict{Symbol,Any}(:a => (10,:red)),'W', [:a]) == " -W10,red"
	@test_throws ErrorException("Nonsense in W option") GMT.opt_pen(Dict{Symbol,Any}(:a => [1 2]),'W', [:a])
	...
	GMT.add_opt_cpt(Dict{Symbol,Any}(:a=>1), "", [:b], 'A', 0, nothing, nothing, false, true, "-T0/10/1");
	GMT.add_opt_cpt(Dict{Symbol,Any}(:a=>:red), "", [:a], 'A', 0, nothing, nothing, false, true, "-T0/10/1");
	...
	@test GMT.helper_decorated(Dict{Symbol,Any}(:line => [1 2 3 4; 5 6 7 8]), true) == "l1/2/3/4,5/6/7/8"
	@test GMT.helper_decorated(Dict{Symbol,Any}(:Line => "CT/CB"), true) == "LCT/CB"
	@test GMT.helper_decorated(Dict{Symbol,Any}(:Line => (1,2,"CB")), true) == "L1/2/CB"
	@test GMT.helper_decorated(Dict{Symbol,Any}(:N_labels => 5), true) == "N5"
	@test_throws ErrorException("DECORATED: 'line' option. When array, it must be an Mx4 one") GMT.helper_decorated(Dict{Symbol,Any}(:line => [1 2 3]), true)
	@test_throws ErrorException("DECORATED: 'dist' (or 'distance') option. Unknown data type.") GMT.helper_decorated(Dict{Symbol,Any}(:dist => (a=1,)))
	@test GMT.parse_quoted(Dict{Symbol,Any}(:label => "aa"), "") == "+Laa"
	@test GMT.parse_quoted(Dict{Symbol,Any}(:label => :header), "") == "+Lh"
	@test GMT.parse_quoted(Dict{Symbol,Any}(:label => :input), "") == "+Lf"
	@test_throws ErrorException("Wrong content for the :label option. Must be only :header or :input") GMT.parse_quoted(Dict{Symbol,Any}(:label => :x), "")
	@test_throws ErrorException("Wrong content for the :label option. Must be only :plot_dist or :map_dist") GMT.parse_quoted(Dict{Symbol,Any}(:label => (:x,)), "")
	GMT.parse_quoted(Dict{Symbol,Any}(:label => 1),"")		# Trigger a warning
	GMT.parse_ls_code!(Dict{Symbol,Any}(:ls => "-"))
	GMT.helper_arrows(Dict{Symbol,Any}(:geovec => "bla"));
	GMT.helper_arrows(Dict{Symbol,Any}(:vecmap => "bla"));

	GMT.parse_paper(Dict{Symbol,Any}(:paper => :g))
	GMT.parse_paper(Dict{Symbol,Any}(:paper => (:i,:g)))
	GMT.leave_paper_mode()

	GMT.add_opt_module(Dict{Symbol,Any}(:coast => 1))
	...
	@test GMT.build_pen(Dict{Symbol,Any}(:lw => 1, :lc => [1,2,3])) == "1,1/2/3"
	...
	d=Dict{Symbol,Any}(:xaxis => (axes=:WSen,title=:aiai, label=:ai, annot=:auto, ticks=[], grid=10, annot_unit=:ISOweek,seclabel=:BlaBla), :xaxis2=>(annot=5,ticks=1), :yaxis=>(custom="lixo.txt",), :yaxis2=>(annot=2,));
	@test GMT.parse_B(d, "")[1] == " -Bsya2 -Bsxa5f1 -Bpyclixo.txt -BpxaUfg10 -Bpx+lai+sBlaBla -BWSen+taiai"
	@test GMT.parse_B(Dict{Symbol,Any}(:B=>:same), "")[1] == " -B"
	@test GMT.parse_B(Dict{Symbol,Any}(:title => :bla), "")[1] == " -Baf -BWSen+tbla"
	@test GMT.parse_B(Dict{Symbol,Any}(:frame => :auto, :title => :bla), "")[1] == " -Baf -BWSen+tbla"
	@test GMT.parse_B(Dict{Symbol,Any}(:B=>:WS), "")[1] == " -Baf -BWS"
	@test GMT.parse_B(Dict{Symbol,Any}(:title => "bla"), "", " -Baf")[1] == " -Baf -B+tbla"
	@test GMT.parse_B(Dict{Symbol,Any}(:frame => (annot=10, title="Ai Ai"), :grid => (pen=2, x=10, y=20)), "", " -Baf -BWSen")[1] == " -Bpa10 -Byg20 -Bxg10 -BWSen+t\"Ai Ai\""
	@test GMT.parse_B(Dict{Symbol,Any}(:frame => (axes=(:left_full, :bottom_full, :right_full, :top_full), annot=10)), "")[1] == " -Bpa10 -BWSEN"
	@test GMT.parse_B(Dict{Symbol,Any}(:xaxis => (axes=:full, annot=10)), "")[1] == " -Bpxa10 -BWSEN"
	@test GMT.parse_B(Dict{Symbol,Any}(:xaxis => "xg10", :yaxis => "g20"), "")[1] == " -Bpxg10 -Bpyg20"
	@test GMT.parse_B(Dict{Symbol,Any}(:frame => (fill=220,)), "", " -Baf -Bg -BWSne")[1] == " -Baf -Bg -BWSne+g220"
	@test GMT.parse_B(Dict{Symbol,Any}(:frame => :full), "")[1] == " -Baf -BWSEN"
	@test GMT.parse_B(Dict{Symbol,Any}(:title => "BlaBla", :frame => :none), "")[1] == " -B+tBlaBla"
	...
	@test GMT.arg_in_slot(Dict{Symbol,Any}(:A => [1 2]), "", [:A], Matrix, [88], nothing) == (" -A", [88], [1 2])
	@test GMT.arg_in_slot(Dict{Symbol,Any}(:A => "ai"), "", [:A], Matrix, nothing, nothing) == (" -Aai", nothing, nothing)
	GMT.arg_in_slot(Dict{Symbol,Any}(:A => "ai"), "", [:A], Matrix, nothing, nothing, nothing)
	GMT.arg_in_slot(Dict{Symbol,Any}(:A => [1 2]), "", [:A], Matrix, [88], nothing, nothing)
	GMT.arg_in_slot(Dict{Symbol,Any}(:A => "ai"), "", [:A], Matrix, nothing, nothing, nothing, nothing)
	GMT.arg_in_slot(Dict{Symbol,Any}(:A => [1 2]), "", [:A], Matrix, [88], nothing, nothing, nothing)
	@test_throws ErrorException("Wrong data type (UnionAll) for option A") GMT.arg_in_slot(Dict{Symbol,Any}(:A => Complex), "", [:A], Matrix, nothing, nothing)
	@test_throws ErrorException("Wrong data type (UnionAll) for option A") GMT.arg_in_slot(Dict{Symbol,Any}(:A => Complex), "", [:A], Matrix, nothing, nothing, nothing)
	@test_throws ErrorException("Wrong data type (UnionAll) for option A") GMT.arg_in_slot(Dict{Symbol,Any}(:A => Complex), "", [:A], Matrix, nothing, nothing, nothing, nothing)

	d=Dict{Symbol,Any}(:L => (pen=(lw=10,lc=:red),) );
	@test GMT.add_opt(d, "", "", [:L], (pen=("+p",GMT.add_opt_pen),) ) == "+p10,red"
	@test GMT.add_opt_pen(Dict{Symbol,Any}(:pen => (width=0.1, style=".")), [:W :pen]) == "0.1,,."
	...
	psxy!("", [0 0; 1 1.1], Vd=dbg2);
	GMT.get_marker_name(Dict{Symbol,Any}(:y => "y"), nothing, [:y], false)
	@test GMT.line_decorated_with_symbol(Dict{Symbol,Any}()) == " -S~d0.88:+sc0.11+gwhite+p0.75,black"
	@test_throws ErrorException("Argument of the *bar* keyword can be only a string or a NamedTuple.") GMT.parse_bar_cmd(Dict{Symbol,Any}(:a => 0), :a, "", "")
	...
	@test GMT.parse_i(Dict{Symbol,Any}(:i=>(0,1,2,2)), "")[1] == " -i0,1,2,2"
	@test GMT.parse_j(Dict{Symbol,Any}(:spherical_dist => "f"), "")[1] == " -jf"
	@test GMT.parse_t(Dict{Symbol,Any}(:t=>0.2), "")[1] == " -t20.0"
	@test GMT.parse_t(Dict{Symbol,Any}(:t=>20), "")[1]  == " -t20.0"
	@test GMT.parse_contour_AGTW(Dict{Symbol,Any}(:A => [1]), "")[1] == " -A1,"
	GMT.helper2_axes("");
	...
	@test GMT.axis("zz", Dict{Symbol,Any}())[1] == " -Bzz"
	@test GMT.axis(Dict{Symbol,Any}(), axes=:WESNZ, annot=(1,2,3), ticks=(4,5,6), grid=(1,2))[1] == " -Bpxa1f4g1 -Bpya2f5g1 -Bza3f6g2 -BWESNZ"

	@test GMT.parse_grid(Dict{Symbol,Any}(), (z=25,)) == " -Bzg25"
	@test GMT.parse_grid(Dict{Symbol,Any}(), ()) == " -Bg"
	@test GMT.parse_grid(Dict{Symbol,Any}(), (xyz=1,)) == " -Bg -Bzg"
	@test GMT.parse_grid(Dict{Symbol,Any}(), "x") == " -Bxg"
	@test GMT.parse_grid(Dict{Symbol,Any}(), "x25") == " -Bxg25"
	@test GMT.parse_grid(Dict{Symbol,Any}(), "y25") == " -Byg25"
	@test GMT.parse_grid(Dict{Symbol,Any}(), "xyz") == " -Bg -Bzg"
	@test GMT.parse_grid(Dict{Symbol,Any}(), :on) == " -Bg"
	...
	@test (GMT.check_flipaxes(Dict{Symbol,Any}(:flipaxes => (xy=1,)), "?/?") == "-?/-?")
	@test (GMT.check_flipaxes(Dict{Symbol,Any}(:flipaxes => ("xy")), "?/?") == "-?/-?")
	@test (GMT.check_flipaxes(Dict{Symbol,Any}(:flipaxes => ("y")), "?/?") == "?/-?")

	@test (GMT.parse_opt_range(Dict{Symbol,Any}(:T => (1,2,0.1,:num)), "", "")[1] == "1/2/0.1+n")
	@test (GMT.parse_opt_range(Dict{Symbol,Any}(:T => (1,2,0.1,:num)), "", "T")[1] == " -T1/2/0.1+n")
	@test (GMT.parse_opt_range(Dict{Symbol,Any}(:T => (1,2,0.1,:log1)), "")[1] == "1/2/0.1+l")
	@test (GMT.parse_opt_range(Dict{Symbol,Any}(:T => (:red, :green, :blue)), "")[1] == "red,green,blue")
	GMT.parse_opt_range(Dict{Symbol,Any}(:T => (1,2,0.1,:mum,:log2)), "")[1]	# Prints a warning
	...
	GMT.guess_WESN(Dict{Symbol,Any}(:p=>(350,2)), "")
	GMT.guess_WESN(Dict{Symbol,Any}(:p=>"350/2"), "")
	GMT.parse_q(Dict{Symbol,Any}(:p=>(350,2)), "")
	...
	GMT.parse_opt_S(Dict{Symbol,Any}(:size => (exp, [1 2])), rand(4))
	GMT.parse_opt_S(Dict{Symbol,Any}(:size => ((pow,2), [1 2])), rand(4))
